$(function () {
    ...
    $(".addre").on("click", ".setDefault", function () {
        $(this).next().remove();
        $(this).parent("p").prev().append('<span class="default">[Default Address]</span>').parents(".addre").addClass("on").siblings().removeClass("on").find(".default").remove().end().find(".tit p").eq(1).prepend('<a href="#" class="setDefault">Set Default</a><span>|</span>');
    ...
    function b() {
        ...
        $(".ok span").text(a);
        if (a == 0) {
            window.location.href = ("doorderselect")
        }
    }
