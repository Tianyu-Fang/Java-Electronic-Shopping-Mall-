...
this.showMoreDay = false; //if to show the dates of previous and next month
this.Obj = objName;		
this.date = null;
...
str += '<div Author="alin" class="cdrWatermark" id="cdrWatermark"></div><div id="cdrBody" style="position:absolute;left:0px;top:0px;z-index:2;width:280px;">';
str += this.getHeader();
str += this.getBody();   
...
str += '.calendar{position:absolute;width:280px!important;width /**/:142px;height:184px!important;height /**/:174px;background-color:'+this.style.bodyBackColor+';border:1px solid ' + this.style.borderColor + ';left:0px;top:0px;z-index:9999;}\n';
str += '.cdrHeader{background-color:'+ this.style.headerBackColor +';width:280px;height:22px;font-size:12px;color:'+this.style.headerFontColor+';}\n';
str += '.cdrWatermark{position:absolute;left:0px;top:55px;width:280px;font-family: Arial Black;font-size:50px;color:'+this.style.watermarkColor+';z-index:1;text-align:center;}\n';
str += '.cdrBodyBar{background-color:' + this.style.bodyBarBackColor + ';font-size:12px;color:' + this.style.bodyBarFontColor +';width:280px;height:20px;}\n';
str += '.cdrBody{width:280px;height:122px!important; height /**/:110px;font-size:12px;cursor:pointer;color:' + this.style.bodyFontColor + ';}\n';
...
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousYear" title="previous year" style="text-align:left; cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeYear(false);"><<</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" id="currentYear" style="width:50px; text-align:center;" onclick="' + this.Obj + '.showMenu(true);" onmouseout="' + this.Obj + '.hideMenu();this.className=\'\';">0</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextYear" title="next year" style="text-align:right; cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeYear(true);">>></td></tr>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="previousMonth" title="previous month" style="cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeMonth(false);"><</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" id="currentMonth" onclick="' + this.Obj + '.showMenu(false);" style="text-align:center;" onmouseout="' + this.Obj + '.hideMenu();this.className=\'\';">0</td>\n';
str += '<td Author="alin" onmouseover="this.className=\'headerOver\'" onmouseout="this.className=\'\'" id="nextMonth" title="next month" style="text-align:right; cursor:pointer;width:10px;" onclick="'+this.Obj+'.onChangeMonth(true);">></td>\n';
...
for(j = 0; j < 7; j++)
{
str += '<td Author="alin" style="text-align:center"class="dayOut" id="cdrDay'+(n++)+'"width="15%"></td>\n';
}
str += '</tr>';
}
str += '</table>\n';
str += '<table Author="alin" class="cdrBodyBar"  cellSpacing="2" cellPadding="0"><tr align="center" Author="alin"><td  Author="alin" style="cursor:pointer;text-align:center;" onclick="'+this.Obj+'.getToday();">Today：'+new Date().toFormatString("yyyy/mm/dd")+'</td></tr></table>\n';
return str;
};
...
for(i = 0; i < 7; i++)
{
str += '<td Author="alin_bar" style="text-align:center; width:15%;">' + day[i] + '</td>\n';     
}
str += '</tr></table>';
return str;  o9ik
}
...
str += '<tr Author="alin" align="center"><td style="text-align:left;" Author="alin" onmouseover="this.className=\'menuOver\'" onmouseout="this.className=\'\'" onclick="'+this.Obj+'.getYearMenu('+ (year - 10) + ')"><<</td>\n';
str += '<td style="text-align:right;" Author="alin" onmouseover="this.className=\'menuOver\'" onmouseout="this.className=\'\'" onclick="'+this.Obj+'.getYearMenu('+ (year + 10) +')">>></td><tr>\n';
str += '</table></td></tr>\n';
str += '</table>';
...
inputObj = _date;
targetObj = _evObj
if(!_date){alert("Wrong parameters!"); return;}
...
{
alert("Exceed the range（1000-9999）!");
}
